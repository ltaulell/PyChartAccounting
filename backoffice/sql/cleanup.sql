-- $Id$
-- drop everything, testing era
--drop table if exists last_insertion cascade;
drop table if exists history cascade;
drop table if exists job_ cascade;
drop table if exists users_in_metagroupes cascade;
drop table if exists groupes_in_metagroupes cascade;
drop table if exists hosts_in_clusters cascade;
drop table if exists hosts_in_queues cascade;
drop table if exists users_in_groupes cascade;
drop table if exists queues cascade;
drop table if exists clusters cascade;
drop table if exists metagroupes cascade;
drop table if exists groupes cascade;
drop table if exists hosts cascade;
drop table if exists users cascade;
